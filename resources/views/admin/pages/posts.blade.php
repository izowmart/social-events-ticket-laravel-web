...
@section('title')
    <title>Posts - Admin Fika Places</title>
@endsection
              ...
              <table class="table table-hover table-bordered" id="adminsTable">
                ...
                <tbody>
                    ...
                    <tr class="item">
                        ...
                        <td>{{$post->comment}}</td>
                        <td><a href="{{url( ($post->media_type == 1 ? 'uploads/posts/images/' : 'uploads/posts/videos/').$post->media_url)}}" target="_blank" class="btn btn-sm btn-outline-primary">View</a></td>
                        <td>
                            @if ($post->status==1)
                                {{'active'}}
